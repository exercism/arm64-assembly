...
def gen_func_body(prop, inp, expected):
    ...
    str_list = []
    str_list.append(f'const {input_element_type} integers[] = {array_literal(integers)};\n')
    str_list.append(f'const {output_element_type} expected[] = {array_literal(expected)};\n')
    str_list.append(f'{output_element_type} actual[MAX_ARRAY_SIZE];\n')
    str_list.append(f'const size_t size = {prop}(actual, integers, ARRAY_SIZE(integers));\n')
